...
We train on \textbf{E-Flare2.7K} for $40$k iterations on one RTX 3090. The standard residual 3D U-Net shows that physically grounded data alone suffice for effective learning. Details are in the \textbf{Appendix}.
...
Figs.~\ref{fig:qualitative_sim}--\ref{fig:qualitative_real} confirm this trend: baselines leave flare artifacts or over-suppress light sources, while \textbf{E-DeflareNet} removes structured flare and preserves valid events.
...
\textbf{(1)} \emph{No Light}: the target removes the light source;
\textbf{(2)} \emph{Simple}: direct additive fusion;
\textbf{(3)} \emph{Full Model (Ours)}: full physics-guided synthesis.
...
The \emph{No Light} failure (Chamfer: $1.9745$ \vs $1.1368$ for ours) shows that ground truth must preserve the actual light source rather than treat all flare-related events as noise. The gap between \emph{Simple} and \emph{Full Model} (Chamfer: $1.3434$ \vs $1.1368$) shows that additive fusion ignores the physical weighting in Sec.~\ref{ssec:forward_model}, especially in flare-dominated regions where $w_{\mathrm{bg}} \to 0$.
...
These ablations confirm the need to preserve light-source semantics and model non-linear event suppression.
