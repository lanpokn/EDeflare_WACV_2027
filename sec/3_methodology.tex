...
\textbf{Practical Considerations.} If continuous irradiance profiles $\mathbf{I}_{\mathrm{bg}}(t)$ and $\mathbf{I}_{\textcolor{ev_red}{\mathrm{\mathbf{f}}}}(t)$ were known, one could compute $w(t)$ and $\xi_{\mathrm{mix}}$ and evaluate $\mathcal{S}_{\mathrm{evt}}$ exactly. In practice, however, continuous irradiance is unavailable, and real sensors deviate from the ideal $\mathcal{S}_{\mathrm{evt}}$ through threshold, latency, and noise effects that are hard to calibrate. Thus, $\mathbf{I}(t)$, the weights, and residuals cannot be reliably recovered; full integral evaluation is neither accessible nor practically meaningful, motivating the discrete approximation in Sec.~\ref{ssec:simulator}.
