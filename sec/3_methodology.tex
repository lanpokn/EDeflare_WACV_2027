...
\textbf{Practical Considerations.} If continuous irradiance profiles $\mathbf{I}_{\mathrm{bg}}(t)$ and $\mathbf{I}_{\textcolor{ev_red}{\mathrm{\mathbf{f}}}}(t)$ were known, one could compute $w(t)$ and $\xi_{\mathrm{mix}}$ and evaluate $\mathcal{S}_{\mathrm{evt}}$ exactly. In practice, however, complete and accurate irradiance information is unavailable, and real event sensors introduce threshold mismatch, latency, noise, and other deviations from the ideal $\mathcal{S}_{\mathrm{evt}}$ that are difficult to calibrate exactly. Thus, $\mathbf{I}(t)$, the weights, and the residual terms cannot be recovered, making a full evaluation of the integral both inaccessible and of limited practical meaning; this motivates the discrete approximation in Sec.~\ref{ssec:simulator}.
